...
are detached, deeply frozen snapshots, so a subscriber cannot accidentally
mutate pi-agents' internal run state or the event seen by later subscribers. A
matching `pi-agents:ready` signal announces the protocol and package version at
session start.
