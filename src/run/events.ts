...
/** Cap for values embedded in persisted events. Full values stay in memory. */
export const MAX_PERSISTED_VALUE_CHARS = 16_000;

/**
 * Truncate a JSON value for persistence. Values that serialize within the cap
 * pass through unchanged; larger ones are replaced by a truncated-string
 * representation with a marker.
 */
export function truncateValue(
  value: unknown,
  cap = MAX_PERSISTED_VALUE_CHARS,
): unknown {
  ...
  const overflow = serialized.length - cap;
  return `${serialized.slice(0, cap)}… [truncated ${overflow} characters]`;
}

/** Copy an event with its embedded values truncated for persistence. */
export function truncateEventForPersistence(event: RunEvent): RunEvent {
  switch (event.type) {
    case "node_completed":
      return { ...event, value: truncateValue(event.value) };
    case "run_completed":
      return { ...event, value: truncateValue(event.value) };
    default:
      return event;
